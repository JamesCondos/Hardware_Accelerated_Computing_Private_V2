...
#pragma HLS ARRAY_PARTITION variable=layer1_output_tile complete dim=1
 ...
 OUT_STATIONARY_CONV1:
    for (int out_feat = 0; out_feat < 64; out_feat++) {
        VITIS_LOOP_41_1: for (int i = 0; i < 17; i++) {
            VITIS_LOOP_42_2: for (int j = 0; j < 17; j++) {
...
#pragma HLS UNROLL FACTOR=4
 int pad_h = (int)fminf(fmaxf(th + kh - P, 0), 17 - 1);
    ...
    STREAM_OUT_CONV1:
    for (int feat = 0; feat < 64; feat++) {
        VITIS_LOOP_80_3: for (int th = 0; th < 17; th++) {
            VITIS_LOOP_81_4: for (int tw = 0; tw < 17; tw++) {
