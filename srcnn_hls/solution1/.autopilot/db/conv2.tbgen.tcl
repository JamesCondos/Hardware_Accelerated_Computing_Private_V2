set moduleName conv2
set isTopModule 0
set isCombinational 0
set isDatapathOnly 0
set isPipelined 1
set pipeline_type none
set FunctionProtocol ap_ctrl_hs
set isOneStateSeq 0
set ProfileFlag 0
set StallSigGenFlag 0
set isEnableWaveformDebug 1
set hasInterrupt 0
set DLRegFirstOffset 0
set DLRegItemOffset 0
set C_modelName {conv2}
set C_modelType { void 0 }
set C_modelArgList {
	{ conv2_weights int 32 regular {array 2048 { 1 1 } 1 1 }  }
	{ conv2_biases int 32 regular {array 32 { 1 3 } 1 1 }  }
	{ layer1_output float 32 regular {array 4161600 { 1 1 } 1 1 } {global 0}  }
	{ layer2_output float 32 regular {array 2080800 { 0 3 } 0 1 } {global 1}  }
}
set hasAXIMCache 0
set C_modelArgMapList {[ 
	{ "Name" : "conv2_weights", "interface" : "memory", "bitwidth" : 32, "direction" : "READONLY"} , 
 	{ "Name" : "conv2_biases", "interface" : "memory", "bitwidth" : 32, "direction" : "READONLY"} , 
 	{ "Name" : "layer1_output", "interface" : "memory", "bitwidth" : 32, "direction" : "READONLY", "extern" : 0} , 
 	{ "Name" : "layer2_output", "interface" : "memory", "bitwidth" : 32, "direction" : "WRITEONLY", "extern" : 0} ]}
# RTL Port declarations: 
set portNum 37
set portList { 
	{ ap_clk sc_in sc_logic 1 clock -1 } 
	{ ap_rst sc_in sc_logic 1 reset -1 active_high_sync } 
	{ ap_start sc_in sc_logic 1 start -1 } 
	{ ap_done sc_out sc_logic 1 predone -1 } 
	{ ap_idle sc_out sc_logic 1 done -1 } 
	{ ap_ready sc_out sc_logic 1 ready -1 } 
	{ conv2_weights_address0 sc_out sc_lv 11 signal 0 } 
	{ conv2_weights_ce0 sc_out sc_logic 1 signal 0 } 
	{ conv2_weights_q0 sc_in sc_lv 32 signal 0 } 
	{ conv2_weights_address1 sc_out sc_lv 11 signal 0 } 
	{ conv2_weights_ce1 sc_out sc_logic 1 signal 0 } 
	{ conv2_weights_q1 sc_in sc_lv 32 signal 0 } 
	{ conv2_biases_address0 sc_out sc_lv 5 signal 1 } 
	{ conv2_biases_ce0 sc_out sc_logic 1 signal 1 } 
	{ conv2_biases_q0 sc_in sc_lv 32 signal 1 } 
	{ layer1_output_address0 sc_out sc_lv 22 signal 2 } 
	{ layer1_output_ce0 sc_out sc_logic 1 signal 2 } 
	{ layer1_output_q0 sc_in sc_lv 32 signal 2 } 
	{ layer1_output_address1 sc_out sc_lv 22 signal 2 } 
	{ layer1_output_ce1 sc_out sc_logic 1 signal 2 } 
	{ layer1_output_q1 sc_in sc_lv 32 signal 2 } 
	{ layer2_output_address0 sc_out sc_lv 21 signal 3 } 
	{ layer2_output_ce0 sc_out sc_logic 1 signal 3 } 
	{ layer2_output_we0 sc_out sc_logic 1 signal 3 } 
	{ layer2_output_d0 sc_out sc_lv 32 signal 3 } 
	{ grp_fu_97_p_din0 sc_out sc_lv 32 signal -1 } 
	{ grp_fu_97_p_din1 sc_out sc_lv 32 signal -1 } 
	{ grp_fu_97_p_opcode sc_out sc_lv 2 signal -1 } 
	{ grp_fu_97_p_dout0 sc_in sc_lv 32 signal -1 } 
	{ grp_fu_97_p_ce sc_out sc_logic 1 signal -1 } 
	{ grp_fu_101_p_din0 sc_out sc_lv 32 signal -1 } 
	{ grp_fu_101_p_din1 sc_out sc_lv 32 signal -1 } 
	{ grp_fu_101_p_dout0 sc_in sc_lv 32 signal -1 } 
	{ grp_fu_101_p_ce sc_out sc_logic 1 signal -1 } 
	{ grp_fu_108_p_din0 sc_out sc_lv 32 signal -1 } 
	{ grp_fu_108_p_dout0 sc_in sc_lv 64 signal -1 } 
	{ grp_fu_108_p_ce sc_out sc_logic 1 signal -1 } 
}
set NewPortList {[ 
	{ "name": "ap_clk", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "clock", "bundle":{"name": "ap_clk", "role": "default" }} , 
 	{ "name": "ap_rst", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "reset", "bundle":{"name": "ap_rst", "role": "default" }} , 
 	{ "name": "ap_start", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "start", "bundle":{"name": "ap_start", "role": "default" }} , 
 	{ "name": "ap_done", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "predone", "bundle":{"name": "ap_done", "role": "default" }} , 
 	{ "name": "ap_idle", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "done", "bundle":{"name": "ap_idle", "role": "default" }} , 
 	{ "name": "ap_ready", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "ready", "bundle":{"name": "ap_ready", "role": "default" }} , 
 	{ "name": "conv2_weights_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":11, "type": "signal", "bundle":{"name": "conv2_weights", "role": "address0" }} , 
 	{ "name": "conv2_weights_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "conv2_weights", "role": "ce0" }} , 
 	{ "name": "conv2_weights_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "conv2_weights", "role": "q0" }} , 
 	{ "name": "conv2_weights_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":11, "type": "signal", "bundle":{"name": "conv2_weights", "role": "address1" }} , 
 	{ "name": "conv2_weights_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "conv2_weights", "role": "ce1" }} , 
 	{ "name": "conv2_weights_q1", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "conv2_weights", "role": "q1" }} , 
 	{ "name": "conv2_biases_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":5, "type": "signal", "bundle":{"name": "conv2_biases", "role": "address0" }} , 
 	{ "name": "conv2_biases_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "conv2_biases", "role": "ce0" }} , 
 	{ "name": "conv2_biases_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "conv2_biases", "role": "q0" }} , 
 	{ "name": "layer1_output_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":22, "type": "signal", "bundle":{"name": "layer1_output", "role": "address0" }} , 
 	{ "name": "layer1_output_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "layer1_output", "role": "ce0" }} , 
 	{ "name": "layer1_output_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "layer1_output", "role": "q0" }} , 
 	{ "name": "layer1_output_address1", "direction": "out", "datatype": "sc_lv", "bitwidth":22, "type": "signal", "bundle":{"name": "layer1_output", "role": "address1" }} , 
 	{ "name": "layer1_output_ce1", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "layer1_output", "role": "ce1" }} , 
 	{ "name": "layer1_output_q1", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "layer1_output", "role": "q1" }} , 
 	{ "name": "layer2_output_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":21, "type": "signal", "bundle":{"name": "layer2_output", "role": "address0" }} , 
 	{ "name": "layer2_output_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "layer2_output", "role": "ce0" }} , 
 	{ "name": "layer2_output_we0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "layer2_output", "role": "we0" }} , 
 	{ "name": "layer2_output_d0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "layer2_output", "role": "d0" }} , 
 	{ "name": "grp_fu_97_p_din0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_97_p_din0", "role": "default" }} , 
 	{ "name": "grp_fu_97_p_din1", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_97_p_din1", "role": "default" }} , 
 	{ "name": "grp_fu_97_p_opcode", "direction": "out", "datatype": "sc_lv", "bitwidth":2, "type": "signal", "bundle":{"name": "grp_fu_97_p_opcode", "role": "default" }} , 
 	{ "name": "grp_fu_97_p_dout0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_97_p_dout0", "role": "default" }} , 
 	{ "name": "grp_fu_97_p_ce", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "grp_fu_97_p_ce", "role": "default" }} , 
 	{ "name": "grp_fu_101_p_din0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_101_p_din0", "role": "default" }} , 
 	{ "name": "grp_fu_101_p_din1", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_101_p_din1", "role": "default" }} , 
 	{ "name": "grp_fu_101_p_dout0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_101_p_dout0", "role": "default" }} , 
 	{ "name": "grp_fu_101_p_ce", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "grp_fu_101_p_ce", "role": "default" }} , 
 	{ "name": "grp_fu_108_p_din0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "grp_fu_108_p_din0", "role": "default" }} , 
 	{ "name": "grp_fu_108_p_dout0", "direction": "in", "datatype": "sc_lv", "bitwidth":64, "type": "signal", "bundle":{"name": "grp_fu_108_p_dout0", "role": "default" }} , 
 	{ "name": "grp_fu_108_p_ce", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "grp_fu_108_p_ce", "role": "default" }}  ]}

set RtlHierarchyInfo {[
	{"ID" : "0", "Level" : "0", "Path" : "`AUTOTB_DUT_INST", "Parent" : "", "Child" : ["1", "2", "3", "4"],
		"CDFG" : "conv2",
		"Protocol" : "ap_ctrl_hs",
		"ControlExist" : "1", "ap_start" : "1", "ap_ready" : "1", "ap_done" : "1", "ap_continue" : "0", "ap_idle" : "1", "real_start" : "0",
		"Pipeline" : "None", "UnalignedPipeline" : "0", "RewindPipeline" : "0", "ProcessNetwork" : "0",
		"II" : "0",
		"VariableLatency" : "1", "ExactLatency" : "-1", "EstimateLatencyMin" : "66585846", "EstimateLatencyMax" : "66585846",
		"Combinational" : "0",
		"Datapath" : "0",
		"ClockEnable" : "0",
		"HasSubDataflow" : "0",
		"InDataflowNetwork" : "0",
		"HasNonBlockingOperation" : "0",
		"IsBlackBox" : "0",
		"Port" : [
			{"Name" : "conv2_weights", "Type" : "Memory", "Direction" : "I"},
			{"Name" : "conv2_biases", "Type" : "Memory", "Direction" : "I"},
			{"Name" : "layer1_output", "Type" : "Memory", "Direction" : "I"},
			{"Name" : "layer2_output", "Type" : "Memory", "Direction" : "O"}],
		"Loop" : [
			{"Name" : "VITIS_LOOP_11_1_VITIS_LOOP_13_2_VITIS_LOOP_15_3", "PipelineType" : "UPC",
				"LoopDec" : {"FSMBitwidth" : "32", "FirstState" : "ap_ST_fsm_pp0_stage1", "FirstStateIter" : "ap_enable_reg_pp0_iter0", "FirstStateBlock" : "ap_block_pp0_stage1_subdone", "LastState" : "ap_ST_fsm_pp0_stage21", "LastStateIter" : "ap_enable_reg_pp0_iter8", "LastStateBlock" : "ap_block_pp0_stage21_subdone", "QuitState" : "ap_ST_fsm_pp0_stage21", "QuitStateIter" : "ap_enable_reg_pp0_iter8", "QuitStateBlock" : "ap_block_pp0_stage21_subdone", "OneDepthLoop" : "0", "has_ap_ctrl" : "1", "has_continue" : "0"}}]},
	{"ID" : "1", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.fadd_32ns_32ns_32_4_full_dsp_1_U13", "Parent" : "0"},
	{"ID" : "2", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.fadd_32ns_32ns_32_4_full_dsp_1_U14", "Parent" : "0"},
	{"ID" : "3", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.fmul_32ns_32ns_32_3_max_dsp_1_U16", "Parent" : "0"},
	{"ID" : "4", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.flow_control_loop_pipe_sequential_init_U", "Parent" : "0"}]}


set ArgLastReadFirstWriteLatency {
	conv2 {
		conv2_weights {Type I LastRead 33 FirstWrite -1}
		conv2_biases {Type I LastRead 257 FirstWrite -1}
		layer1_output {Type I LastRead 42 FirstWrite -1}
		layer2_output {Type O LastRead -1 FirstWrite 277}}}

set hasDtUnsupportedChannel 0

set PerformanceInfo {[
	{"Name" : "Latency", "Min" : "66585846", "Max" : "66585846"}
	, {"Name" : "Interval", "Min" : "66585846", "Max" : "66585846"}
]}

set PipelineEnableSignalInfo {[
	{"Pipeline" : "0", "EnableSignal" : "ap_enable_pp0"}
]}

set Spec2ImplPortList { 
	conv2_weights { ap_memory {  { conv2_weights_address0 mem_address 1 11 }  { conv2_weights_ce0 mem_ce 1 1 }  { conv2_weights_q0 mem_dout 0 32 }  { conv2_weights_address1 MemPortADDR2 1 11 }  { conv2_weights_ce1 MemPortCE2 1 1 }  { conv2_weights_q1 MemPortDOUT2 0 32 } } }
	conv2_biases { ap_memory {  { conv2_biases_address0 mem_address 1 5 }  { conv2_biases_ce0 mem_ce 1 1 }  { conv2_biases_q0 mem_dout 0 32 } } }
	layer1_output { ap_memory {  { layer1_output_address0 mem_address 1 22 }  { layer1_output_ce0 mem_ce 1 1 }  { layer1_output_q0 mem_dout 0 32 }  { layer1_output_address1 MemPortADDR2 1 22 }  { layer1_output_ce1 MemPortCE2 1 1 }  { layer1_output_q1 MemPortDOUT2 0 32 } } }
	layer2_output { ap_memory {  { layer2_output_address0 mem_address 1 21 }  { layer2_output_ce0 mem_ce 1 1 }  { layer2_output_we0 mem_we 1 1 }  { layer2_output_d0 mem_din 1 32 } } }
}
