...
{
 int padding = 5/2;
 VITIS_LOOP_11_1: for (int out_feat = 0; out_feat < 1; out_feat++) {

    VITIS_LOOP_13_2: for (int out_feat_y = 0; out_feat_y < 255; out_feat_y++) {

     VITIS_LOOP_15_3: for (int out_feat_x = 0; out_feat_x < 255; out_feat_x++) {
         ...
         float feat_bias = conv3_biases[out_feat];
         float convolution = 0;

      VITIS_LOOP_26_4: for (int in_feat = 0; in_feat < 32 ; in_feat++) {

       VITIS_LOOP_28_5: for (int kernel_x = 0; kernel_x < 5; kernel_x++) {

        VITIS_LOOP_30_6: for (int kernel_y = 0; kernel_y < 5; kernel_y++) {
         int new_ftmap_height = fmin(fmax(out_feat_y + kernel_y - padding, 0), 255 - 1);
         int new_ftmap_width = fmin(fmax(out_feat_x + kernel_x - padding, 0), 255 - 1);
         convolution += conv3_weights[out_feat][in_feat][kernel_y][kernel_x]*input_ftmap[in_feat][new_ftmap_height][new_ftmap_width];
        }
       }
      }
      output_ftmap[out_feat][out_feat_y][out_feat_x] = fmaxf(0, convolution + feat_bias);
